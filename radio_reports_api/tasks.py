...
from radio_reports_api.task_scripts.get_simplified_reports import get_simplified_report_data_for_languages
...
def generate_report_text_and_model(
    ...
    report_object.processing_status = "Generating simplified reports."
    report_object.save()

    try:
        simplified_reports = get_simplified_report_data_for_languages(report_data, default_report_simplification_languages)
        # print(simplified_reports)
    except:
        task_status = f"Failed to generate simplified reports."
        report_object.processing_status = task_status
        report_object.save()
        print(task_status)
        return task_status
        
    report_object.simplified_reports = json.dumps(simplified_reports)
